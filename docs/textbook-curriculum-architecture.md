...
次工程は、第2章の詳細設計である。ただし、第2章だけを閉じて設計しない。

第2章の設計では、必ず次を意識する。

- 第3章DNS/DHCPでUDPやポート番号を使う
- 第4章Web/TLSでTCP接続とポート番号を使う
- 第9章プロキシ/LBで5タプルやセッションを使う
- 第10章VPNでUDP/500、UDP/4500、ESPの話につながる
- 第11章FWでプロトコル/ポートを条件にした許可・遮断へつながる
- 第18章CoAPでUDPとメッセージIDの話につながる

つまり、第2章は単なるTCP/UDP章ではなく、後続章の共通言語を作る章である。
