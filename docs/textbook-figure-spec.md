...
最終更新: 2026-07-18（第20章 午後問題の読み方・総合演習を追加。**全20章完結**）
...
| 20 | **2枚構成（2026-07-17モック合意）**: ①packet-flow graph stack 流用の**全体図**（4段spine＋上=社外PC・第2拠点・クラウド(leafIds)・中段=DMZのWeb公開/メール・下=PC/IP電話/監視サーバの12ノード。読み方①境界→②経路→③許可→④冗長の4ステップ。④は外側回線`blockedLink`✕で**SPOF読み**＝第11章回収。**FW verdictチップは中段の葉と衝突するため全体図では使わない**） ②**境界クローズアップ**（第10章LB図の骨格＋境界ルータ・社外PC＝spine inet/境界ルータ/FW/LB＋下にWebプール。社外→自社WebへHTTPSの1本を7ステップで全章接続＝DNS(2章)→TCP(3章)→BGP(8章)→FW verdict通過(9章)→VIP(10章)→TLS(4章)。吹き出しは上半分のステップのみ＝図高472でbubblesのanchorクランプ(y≤172)が下半分で浮くため） / record-table（設問3タイプ×見る場所×答え方の型）。実際の午後と同じく代表機器のみ描き「図に無い機器は問われない」をcalloutで明示。新kind/新role/engine変更なし。**topology ramp完成＝全20章完結** |
