...
- **文体は敬体（ですます調）を基本とし、言い切れるところは体言止めを使う。** 常体（だ・である・〜する。）と敬体を同じ本文内で混在させない（混在は強い AI 臭の原因）。
- 避ける: 「〜することができます」「〜になります」の連発、無意味な前置き、箇条書きだけで終える節、冗長な敬語の重ね。
- 好む: 短く言い切る、具体例から入る、つまずきポイントを先回りで一言、図へ橋渡しする 1 行。
- 1 節の本文は短く。理解の主役は図と動き。

### 6.1 表記規約（2026-07-04 第1〜8章レビューで確定・全章共通）

- ダッシュは「——」（全角ダーシ2つ・前後スペースなし）に統一。
- 「宛先／あて先」の使い分け: 図中の値・ラベル（ヘッダ表・表セル・パケットチップ・列名）は「宛先」、本文・説明文・caption/takeaway の地の文は「あて先」。
- MACアドレスの例示はハイフン区切り（例 00-11-22-33-44-55）。
- 「3ウェイハンドシェイク」（IPA表記。3way とは書かない）。SYNへの応答は「SYN/ACK」。
- NAPT の初出には「（IPマスカレードとも呼びます）」を添える（IPAの併記に合わせる）。
- IP:ポートのコロン表記（192.168.10.10:51000 等）は、初出の章で一言説明する。
- 同じ言い回し（「押さえれば十分」「材料がそろいました」等）を同一章内・隣接章の同じ位置で繰り返さない。
