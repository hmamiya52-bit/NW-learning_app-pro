...
const arpFigure: PacketFlowFigure = {
  ...
  takeaway: '別ネットワークあてのとき、最初に調べるMACは相手ではなく[[green:デフォルトゲートウェイ]]のもの。',
...
export const ch01Osi: TextbookChapter = {
  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          kind: 'text',
          text: 'では、PCはどうやって「同じLANか、外か」を見分けるのでしょうか。使うのはIPアドレスの[[blue:ネットワーク部]]です。192.168.10.0/24 の「/24」は「先頭24ビットが同じなら同じネットワーク（セグメント）」という意味。PCは相手のIPのこの部分を自分と見比べ、[[green:同じネットワークなら相手へ直接]]、[[blue:違えば出口（デフォルトゲートウェイ）へ]]渡します。ビット単位の計算は第6章です。なお、ここでの「セグメント」はネットワークの区切りのこと。さきほどL4の単位として出た「セグメント」とは、同じ言葉の別の使い方です。',
        },
        {
          kind: 'text',
          ...
          items: [
            {
              ...
              answer: 'デフォルトゲートウェイ（ルータ）のもの。[[blue:あて先IP]]はWebサーバのまま変わらない、という点もあわせて問われます。',
            },
          ],
        },
      ],
  ...
  takeaways: [
    ...
    '送るときは外へ外へと包み、受け取るときは外から順に開きます（カプセル化／デカプセル化）。送って終わりではありません。',
    '[[green:MACはすぐ隣]]・[[blue:IPは最終的な相手]]・[[amber:ポートは相手の中のアプリ]]。',
    '別ネットワークあての最初のあて先MACは、相手ではなくデフォルトゲートウェイ（LANの出口にあるルータ）。',
