...
export const ch07Routing: TextbookChapter = {
  ...
  title: 'ルーティング（経路制御）',
  summary:
    'ネットワークが増えると、どの道を通って届けるかを決める必要が出てきます。その判断のよりどころが経路表です。経路を管理者が手で書く方法と、ルータどうしで学び合う方法（OSPF）の違いが分かると、構成図の線をたどるだけで通信の道筋が読めるようになります。',
  status: 'published',
  estimatedMinutes: 22,
  intro: [
    {
      kind: 'text',
      text: '第1章で、ルータは「あて先IPを見て、次にどの機器へ渡すかを決める機器」として登場しました。では、その道はどうやって選ばれるのか。中身はここからです。',
    },
    {
      kind: 'text',
  ...
  sections: [
    {
      ...
      blocks: [
        ...
        { kind: 'figure', figure: routeTableFigure },
        {
          ...
          tone: 'warn',
          title: '次ホップは「次の1歩」',
          body: '1台のルータが決めるのは「次にどの機器へ渡すか」だけで、最終あて先までの全経路ではありません。あて先IPは基本そのままで、区間ごとに付け替わるのはL2のあて名（MAC）だけ。第1章で見たとおり、各ルータの「次の1歩」が積み重なって、端から端まで届きます。',
        },
      ],
    },
    {
      ...
      blocks: [
        ...
        { kind: 'figure', figure: longestMatchFigure },
        {
          ...
          tone: 'info',
          title: 'デフォルトルートは最後の砦',
          body: '0.0.0.0/0 は「どの行にも一致しないとき」の行き先で、プレフィックスが一番短いので、他に一致があれば必ず負けます。第1章のデフォルトゲートウェイは、経路表ではこの 0.0.0.0/0 として書かれていました。その先（インターネット境界）は第8章です。',
        },
      ],
    },
    {
      ...
      blocks: [
        ...
        { kind: 'figure', figure: staticRouteFigure },
        {
          ...
          body: '普段は使わない予備の経路に、わざと大きな距離（メトリック）を付けておき、主経路が消えたときだけ使う書き方を、フローティングスタティックと呼びます。名前だけ知っていれば、先へ進めます。',
        },
      ],
    },
    {
      ...
      blocks: [
        ...
        {
          kind: 'text',
          text: 'OSPFはリンクステート型です。各ルータが「どこと、どれだけの太さ（帯域）でつながっているか」を配り合い、全員が同じ全体地図を持ちます。その地図から、[[blue:コスト（帯域から決まる距離）が最小]]の経路を、各自が計算します。',
        },
        {
          kind: 'text',
        ...
        { kind: 'figure', figure: ospfFigure },
        {
          kind: 'callout',
          tone: 'tip',
          title: 'スタティックとOSPFの使い分け',
          body: '手動のスタティックは小規模で確実、自動のOSPFは変化に強い反面、設計が要ります。台数が少なく構成が変わらないなら手で書き、拠点や経路が多いならOSPFに任せる、という選び方になります。',
        },
        {
          kind: 'text',
          text: 'OSPFのルータは、まず[[blue:隣のルータと知り合いになる]]ところから始めます（隣接関係）。知り合いになった相手とだけ地図の情報を配り合い、全員の地図が一致して経路が定まった状態を[[blue:収束]]と呼びます。構成が変わると一時的に地図がずれるので、収束するまでは通信が乱れることがあります。',
        },
        {
          kind: 'callout',
          tone: 'warn',
          title: '誰とでも知り合いになってしまう危うさ',
          body: 'そのままだと、ネットワークにつながれた[[red:素性の分からないルータ]]とも隣接関係を結び、でたらめな経路を教え込まれてしまいます。防ぐには、隣接を結ぶ相手を認証したり、端末しかいないポートでは[[blue:OSPFの情報を流さない]]設定にします。科目Bでは「この設定をしないと何が起きるか」の形で問われます。',
        },
        {
          kind: 'text',
          text: 'ルータの台数が増えると、全員が全体の地図を持つのは重くなります。そこで大きなネットワークでは、地図を配る範囲を[[blue:エリア]]という単位で区切ります。エリアの中の細かい構成は外へ流さず、代わりにまとめた情報だけを渡すので、地図も計算も軽くなります。',
        },
      ],
    },
    {
    ...
    },
    {
      ...
      blocks: [
        {
          kind: 'text',
          text: '経路表には、同じあて先が複数の経路源（直結・スタティック・OSPF）から届くこともあります。このときは、まず[[blue:AD（アドミニストレーティブディスタンス）]]、つまり経路源の信頼度＝優先順位で選び、同じ経路源どうしなら[[blue:メトリック]]（距離）で選びます。',
        },
        { kind: 'figure', figure: adFigure },
      ],
    },
    {
      heading: '科目Bでは「経路表を読んで次ホップを追う」',
      blocks: [
        {
          kind: 'text',
          text: 'ネスペ科目Bで最も多いのが、経路表を読んで「この通信は、どのルータのどの行に一致し、次ホップはどこか」を追う問題です。ここまでの図と同じ読み方で解けます。',
        },
        {
          kind: 'text',
          text: 'あわせて、ロンゲストマッチでの一致行の選択、冗長経路の切替（主経路がダウンしたらどこを通るか）、経路集約の読みも、科目Bで頻出です。',
        },
        {
          kind: 'callout',
          tone: 'info',
  ...
  takeaways: [
    ...
    '冗長な2経路があれば、主経路が切れても[[blue:自動で予備へ切替]]（科目B頻出）。',
