...
const overviewFigure: PacketFlowFigure = {
  ...
  steps: [
    ...
    },
    {
      ...
      packetLabel: '',
      headers: { l2: '', l3: '' },
      explanation: 'R1の 0.0.0.0/0（デフォルトルート）の行き先が、この境界ルータ。',
    },
    {
...
const naptFigure: PacketFlowFigure = {
  ...
  steps: [
    ...
    },
    {
      ...
      headers: { l2: '', l3: '' },
      tableRows: NAT_EMPTY,
      explanation: 'R1の経路表では 0.0.0.0/0 に一致（第7章）。次ホップは境界ルータ。',
    },
    {
      focus: { type: 'link', a: 'r1', b: 'br' },
      packetLabel: '送信元 192.168.10.10:51000',
      headers: { l2: '', l3: '' },
      tableRows: NAT_EMPTY,
      explanation: '送信元はまだプライベートIP。世界では通用しないので境界で書き換えます。',
    },
    {
      ...
      tableRows: NAT_ONE,
      tableHighlightRow: 0,
      explanation: '境界ルータが送信元を書き換え（NAPT）、元との対応を変換表に記録します。',
    },
    {
      focus: { type: 'node', id: 'br' },
      packetLabel: '送信元 203.0.113.1:62000',
      ...
      tableRows: NAT_TWO,
      tableHighlightRow: 1,
      explanation: '別フロアPCの通信も外へ。ポートを変えて同じグローバルIPを共有します。',
    },
    {
      ...
      headers: { l2: '', l3: '' },
      tableRows: NAT_TWO,
      explanation: 'グローバルの送信元でインターネットを渡ります。あて先は出発時から不変。',
    },
    {
      ...
      headers: { l2: '', l3: '' },
      tableRows: NAT_TWO,
      explanation: 'サーバが応答。相手から見えるのは、境界ルータの外側の住所だけです。',
    },
    {
      focus: { type: 'node', id: 'br' },
      ...
      tableRows: NAT_TWO,
      tableHighlightRow: 0,
      explanation: '変換表で 62000 は業務PCの通信と判明。あて先を業務PCへ書き戻します。',
    },
    {
  ...
  steps: [
    {
      ...
      packetLabel: '',
      headers: { l2: '', l3: '' },
      explanation: '自社が直接つながるのは契約ISPだけ。外あてはすべてISPへ出します。',
    },
    {
...
export const ch08InternetNatBgp: TextbookChapter = {
  ...
  summary:
    '社内で使っている住所は、そのままでは世界に出られません。社内と世界の橋渡しをするのが境界ルータで、住所を付け替える働きがNAT。その先の道を世界規模で教え合うのがBGPです。第1章の「あて先IPは基本変わらない」に残っていた例外も、ここで回収されます。',
  status: 'published',
  estimatedMinutes: 22,
  intro: [
    {
      kind: 'text',
      text: '第7章まで、パケットの旅はすべて社内で完結していました。ここでいよいよ社外のインターネットへ出ます。',
    },
    {
      kind: 'text',
      text: '必要になるのは、[[blue:世界で通用する住所]]（グローバルIP）、社内の住所との[[blue:橋渡し]]（NAT）、そして世界規模の[[blue:道案内]]（BGP）の3つです。',
    },
  ],
  sections: [
    {
      ...
      blocks: [
        ...
        {
          kind: 'text',
          text: '境界ルータの外側には、世界で通用する[[blue:グローバルIP]]（203.0.113.1）が付きます。その先はISP（接続事業者）の網を経て、世界中のネットワークへ。全体図で追うと、社内のどこから出ても最後はこの1本に集まることが分かります。',
        },
        { kind: 'figure', figure: overviewFigure },
        {
          ...
          body: '社内と社外の境目は、ネスペ科目Bで最もよく問われる場所。第9章ではこの境界にファイアウォールが入り、内部/DMZ/外部の三層になります。いまは「社内の出口は境界ルータ」とだけ分かっていれば足ります。',
        },
      ],
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          body: '内部LAN（192.168.10.0/24）もサーバLAN（172.16.0.0/24）も、プライベートの範囲内。IPv4の住所は全部で約43億個と足りないため、社内はプライベートでまかない、外に出るときだけ橋渡しするのが定石です。その橋渡しが次のNAT。住所を根本から増やす解決策が、第15章のIPv6です。',
        },
      ],
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          body: '変換表の行は、社内から出た通信が作ります。外から勝手に始まる通信は対応する行が無く、境界で行き止まり。公開サーバのように外から受けたい場合は、固定の対応をあらかじめ書く[[blue:静的NAT]]（ポートフォワーディング）を使います。いまは名前だけで十分です。第9章の公開サーバ（DMZ）で本格的に出てきます。',
        },
      ],
    },
    {
    ...
    {
      heading: '科目Bでは「変換表を読んで通信を追う」',
      blocks: [
        {
          kind: 'text',
          text: '科目Bの定番は、NAPTの[[blue:変換表を読ませる問題]]。ある通信の送信元/あて先が、境界の前後でどう変わるかを追います。ここで効くのが、行きと戻りの対称性です。',
        },
  ...
  takeaways: [
    ...
    '第1章の「あて先IPは基本変わらない」の例外が[[blue:NAT]]。行きは送信元・戻りはあて先が書き換わります（科目B頻出）。',
    'インターネットは[[blue:AS]]（組織ごとのネットワークのかたまり）の集まりで、AS間の経路交換が[[blue:BGP]]。名前と役割だけで十分です。',
    '外から始まる通信は変換表に無く、そのままでは入れません。公開サーバに使う[[blue:静的NAT]]は、第9章のDMZとセットで出てきます。',
  ],
  checks: [
    {
