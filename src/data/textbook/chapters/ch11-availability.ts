...
const spofFigure: PacketFlowFigure = {
  ...
  caption: '主要な機器が[[blue:1台ずつ]]の一本道。どれか1台が[[red:故障]]すると、その先へ通信が届きません。',
...
const vrrpFigure: PacketFlowFigure = {
  ...
  caption: '出口のルータを[[blue:2台]]に。仮想IP[[blue:192.168.10.1]]を共有し、稼働側が受け持ちます。',
  takeaway: '故障で待機側が[[blue:同じ仮想IP]]を引き継ぎます。端末のGWは[[blue:仮想IPのまま]]で切替わりに気づきません。',
...
const lagFigure: PacketFlowFigure = {
  ...
  caption: '2本の[[blue:物理リンク]]を束ねて[[blue:1本の論理リンク]]に。帯域が増え、冗長にもなります。',
  takeaway: '片方が[[red:切れて]]も、残り1本が通信を運びます。帯域は半分でも、通信は止まりません。',
  ...
  steps: [
    {
      ...
      packetLabel: '',
      headers: { l2: '', l3: '' },
      explanation: 'L2SWと上位ルータの間を2本のリンクで結び、束ねて1本の論理リンクに。帯域は2Gビット/秒です。',
    },
    {
      focus: { type: 'link', a: 'sw', b: 'rt' },
      ...
      packetLabel: '',
      headers: { l2: '', l3: '' },
      explanation: '片方のリンクが切れても、残る1本が通信を運びます。帯域は半分でも、通信は止まりません。',
    },
  ],
}
    ...
    },
    {
      ...
      blocks: [
        ...
        { kind: 'figure', figure: vrrpFigure },
        {
          ...
          body: 'アクティブが故障しても、スタンバイが[[blue:同じ仮想IP]]を引き継いで稼働に変わります。端末のデフォルトゲートウェイは[[blue:仮想IPのまま]]で、設定を変える必要はありません。第1章で「デフォルトゲートウェイ＝外への出口」を設定しましたが、その正体が、この[[blue:中身が切替わる仮想IP]]です。似た仕組みにHSRPもあります。名前だけ押さえれば十分です。',
        },
      ],
    },
    {
