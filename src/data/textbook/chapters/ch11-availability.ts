...
// §2 VRRPの詳細（優先度・アドバタイズメント・トラッキング）。過去問はVRRP 9件で、
// R4-G2-1「インタフェースの障害を検知した時に優先度を下げる」＝トラッキング、
// R4-G2-2「アドバタイズメントを決められた時間内に受信しなくなる」＝故障の検知、H26-G1-1がBFD連動。
const vrrpDetailTable: RecordTableFigure = {
  kind: 'record-table',
  id: 'ch11-vrrp-detail',
  title: 'VRRPが切り替わるまでの3つの設定',
  caption: '[[blue:どちらが稼働側か]]と[[blue:どう気づくか]]を決める設定です。',
  takeaway: '合図が途切れれば待機側が引き継ぎます。回線側の故障は[[blue:トラッキング]]で優先度を下げて譲ります。',
  rowHeader: true,
  emphasizeKey: 'what',
  columns: [
    { key: 'name', label: '設定' },
    { key: 'what', label: '決めること・すること' },
    { key: 'why', label: 'これが無いと' },
  ],
  rows: [
    {
      name: '優先度',
      what: '値が大きいほうが稼働側（マスタ）になる',
      why: 'どちらが受け持つか決まらない',
    },
    {
      name: 'アドバタイズメント',
      what: '稼働側が定期的に流す生存の合図',
      why: '待機側が故障に気づけない',
    },
    {
      name: 'トラッキング',
      what: '見張っている回線が切れたら自分の優先度を下げる',
      why: '外向きの回線だけ切れても切り替わらない',
    },
  ],
}
  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          kind: 'text',
          text: 'では、2台はどうやって「どちらが稼働側か」を決め、故障にどう気づくのでしょうか。決めるのは[[blue:優先度]]という設定値で、大きいほうが稼働側（マスタ）になります。気づく方法は、稼働側が定期的に流す[[blue:アドバタイズメント]]という生存の合図です。',
        },
        { kind: 'figure', figure: vrrpDetailTable },
        {
          kind: 'text',
          text: 'ここに落とし穴があります。稼働側のルータ自体は生きていても、その先の[[amber:外向きの回線]]が切れていたらどうなるでしょうか。合図は流れ続けるので待機側は気づかず、通信だけが行き止まりになります。これを防ぐのが[[blue:トラッキング]]で、見張っている回線が切れたら自分の優先度を下げ、待機側へ譲ります。',
        },
        {
          kind: 'callout',
          tone: 'warn',
          title: '戻すか、戻さないか（プリエンプト）',
          body: '故障した機器が直ったとき、優先度の高いほうへ自動で戻すかどうかを決めるのが[[blue:プリエンプト]]です。自動で戻せば元の設計どおりになりますが、切替えのたびに通信が一瞬止まります。不安定な機器が復旧と故障を繰り返すと、そのたびに切替わって被害が広がるため、あえて戻さない設定にすることもあります。「どちらが良いか」ではなく「[[blue:なぜそう決めたか]]」を書かせる形で問われます。',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          kind: 'text',
          text: 'ケーブルが抜ければ機器はすぐ気づきます。やっかいなのは、[[amber:リンクは上がったままなのに通信だけが通らない]]状態です。間にある機器の内部で詰まっている場合などです。リンクを見ているだけでは気づけません。',
        },
        {
          kind: 'text',
          text: 'そこで、相手と短い間隔で合図をやり取りし、返ってこなくなったら「切れた」と判断する仕組みを別に置きます。これが[[blue:BFD]]です。VRRPやルーティングの切替えを、この判断と連動させると、切り替わりが目に見えて速くなります。',
        },
        {
          kind: 'callout',
          tone: 'info',
          title: '間にスイッチがあると見抜けない',
          body: 'BFDは[[blue:相手と直接やり取りできる合図]]なので、途中にL2SWをはさむと注意が要ります。L2SWが生きていれば合図は届いてしまい、その先のリンク障害を見抜けません。「どこからどこまでを見張っているのか」を図で確かめるのが、この手の設問の解き方です。',
        },
      ],
    },
    {
