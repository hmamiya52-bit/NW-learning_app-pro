...
export const ch17VirtualizationCloud: TextbookChapter = {
  ...
  summary:
    'サーバの仮想化で1台の物理サーバに複数のVMを動かし、クラウドではVPCという専用の仮想ネットワークを使います。物理網の上に論理ネットワークを重ねるVXLANは、元のイーサネットフレームをUDP/IPで包んで運ぶオーバーレイで、第5章のVLANを大きく広げ、第12章のトンネルの発展形にあたります。制御を集中させるSDNまで、仮想化とクラウドはこの土台の上に成り立っています。',
  status: 'published',
  estimatedMinutes: 16,
  intro: [
    {
      kind: 'text',
      text: 'ここまで見てきた構成図は、どれも物理の機器を1つずつ置いたものでした。ここから先は、サーバもネットワークも[[blue:ソフトウェアで作る]]世界です。物理の台数や配線に縛られず、必要な機器やネットワークを、必要なだけ論理的に用意できます。',
    },
    {
      kind: 'text',
  ...
  sections: [
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          body: 'これまで社内とインターネットだけだった構成図に、[[emerald:クラウドのVPC（10.0.0.0/16）]]が加わります。VPCの中では、いま見たVMが動いています。次は、この離れたVPCと拠点を、物理を意識せずにつなぐ方法です。',
        },
      ],
    },
    {
    ...
    },
    {
      ...
      blocks: [
        ...
        { kind: 'figure', figure: sdnTable },
        {
          kind: 'callout',
          tone: 'info',
          title: 'EVPNは名前と役割だけで足ります',
          body: 'VXLANで「どのMACアドレスがどの出入口の先にいるか」を配る制御の仕組みを[[blue:EVPN]]と呼びます。VXLANが「運ぶ」役、EVPNが「居場所を配る」役という分担です。細かい中身まで覚える必要はなく、名前と役割だけ分かれば足ります。',
        },
      ],
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            {
              question:
                '拠点のサーバが、VXLANでつないだクラウドのVM（10.0.0.11）あてに通信します。物理網（アンダーレイ）を流れているとき、パケットの外側のあて先IPアドレスは、何を指すか。',
              answer:
                'クラウド側の出入口（VTEP）のIPアドレスです。元のあて先である10.0.0.11は、VXLANで包まれた内側にあり、アンダーレイ上では外側の出入口どうしのIPだけが見えます。第12章のIPsecと同じく、外側と内側を分けて読むことが大切です。',
            },
          ],
        },
        {
  ...
  takeaways: [
    ...
    '[[blue:SDN]]は制御（決める）と転送（送る）を分け、コントローラが集中して管理する考え方。EVPNはVXLANの居場所を配る制御で、こちらも名前だけで足ります。',
  ],
  checks: [
    {
