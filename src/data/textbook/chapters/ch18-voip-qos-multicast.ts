...
const qosQueue: PriorityQueueFigure = {
  ...
  caption: '待ち行列に混じった[[emerald:音声]]が、データより先に抜けて回線へ送り出されます。',
  takeaway: '帯域そのものを増やすのではなく、送る順番を変えて、遅れに弱い音声を守る仕組みです。',
  arrivals: [
    { id: 'd1', label: 'データ', kind: 'data' },
    { id: 'v1', label: '音声', kind: 'voice' },
    { id: 'd2', label: 'データ', kind: 'data' },
    { id: 'v2', label: '音声', kind: 'voice' },
    { id: 'd3', label: 'データ', kind: 'data' },
  ],
  queueLabel: '待ち行列（届いた順）',
  sentLabel: '回線へ送出（送った順）',
  gateLabel: '優先度で選ぶ（音声が先）',
  steps: [
    { sent: 0, explanation: '混雑した回線に、音声とデータが順番待ちで並びます。' },
    { sent: 1, explanation: '優先度の高い音声を、行列から先に送り出します。' },
    { sent: 2, explanation: '残りの音声も、データより先に送り出します。' },
    { sent: 5, explanation: '音声を送り終えてから、データを順に送り出します。' },
  ],
}
