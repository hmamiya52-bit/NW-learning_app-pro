...
export const ch19Operations: TextbookChapter = {
  ...
  intro: [
    {
      kind: 'text',
      text: 'ここまでの18章は、ネットワークを[[blue:作る]]話でした。この章からは、作ったものを[[blue:守る]]話に入ります。守るための仕事は2つ。異常に[[blue:気づく]]ことと、原因の場所を[[blue:切り分ける]]ことです。',
    },
    {
      kind: 'text',
  ...
  sections: [
    ...
    {
      heading: '科目Bは切り分けの根拠を言葉にする',
      blocks: [
        {
          kind: 'text',
          text: '科目Bでは、障害の状況から[[blue:どの層のどこが原因か]]を絞らせる問題、監視で[[blue:何を集めておくべきか]]を選ばせる問題、そして[[blue:ログ]]から発生時刻や順序を読み取る問題が定番です。',
        },
        {
          kind: 'check',
          label: '設問例',
          items: [
            {
              question:
                '内部のPCから社外のWebサイトへつながりません。GW（192.168.10.1）へのpingは届き、社外サーバのIPへのpingは届きません。原因はどの範囲に絞れるか。',
              answer:
                'GWより先、社外へ向かう経路の範囲に絞れます。GWへ届いた時点で、PCの設定から同一セグメントまでは無事と確かめられているからです。あとは境界ルータやその先の回線、FWの許可設定を、近い順に確かめます。',
            },
          ],
        },
        {
