...
// 第20章 科目Bの読み方・総合演習。全20章の到達点＝topology ramp の完成形。新概念なし・統合。
...
// 中段=DMZ（Web公開・メール）、下=内部LAN（PC・IP電話・監視サーバ）。実際の科目Bと同じく代表機器のみ描く。
...
const answerTable: RecordTableFigure = {
  ...
  caption: '科目Bの設問は大きく3タイプ。[[blue:どこを見て]]、[[blue:何を根拠に]]書くかの対応表です。',
...
export const ch20AfternoonReading: TextbookChapter = {
  ...
  intro: [
    {
      kind: 'text',
      text: '第1章の構成図は、PCとルータとサーバだけの小さな1枚でした。それが19章分の積み上げで、拠点もクラウドも冗長も監視も含む、科目Bの問題文と同じスケールの図に育っています。',
    },
    {
      kind: 'text',
    ...
    },
    {
      kind: 'callout',
      tone: 'info',
      title: '「午後」と呼ばれていた試験が科目Bです',
      body: '2026年度から試験がCBT方式になり、名前が変わりました。かつての[[blue:午後Ⅰ]]が[[blue:科目B-1]]、[[blue:午後Ⅱ]]が[[blue:科目B-2]]です（午前Ⅰ・午前Ⅱは科目A-1・科目A-2）。問われる範囲・出題形式・問題数・試験時間はそのままなので、少し前の参考書や過去問が「午後」と書いていても、中身は同じものと考えて差し支えありません。',
    },
  ],
  sections: [
    {
      ...
      blocks: [
        {
          kind: 'text',
          text: '科目Bの構成図は情報量が多く、眺めているだけでは頭に入りません。そこで、見る順番を決めておきます。まず[[blue:境界]]（内と外の区切り）、次に[[blue:経路]]（どこを通るか）、そのうえで[[blue:許可]]（何が通れるか）、最後に[[blue:冗長]]（止まったらどうなるか）。前が分からないと次が読めない、依存関係の順です。',
        },
        { kind: 'figure', figure: readingFigure },
        {
          kind: 'text',
          text: '④のように冗長を確かめると、[[red:単一障害点]]（第11章のSPOF）が浮かび上がります。科目Bでは「この構成の弱点はどこか」という形でよく問われます。',
        },
        {
          kind: 'check',
          label: '設問例',
    ...
    },
    {
      ...
      blocks: [
        {
          kind: 'text',
          text: '型で全体を掴んだら、今度は通信を端から端まで追います。題材は科目Bの定番、社外の利用者から自社の公開Webサイトへの[[blue:HTTPS]]。名前解決から負荷分散まで、この1本に各章の主役が順に登場します。',
        },
        ...
        {
          kind: 'text',
          text: '科目Bの問題も、実はこの「抜き出し」をやっています。大きな図から設問に関係する区間だけを頭の中で抜き出し、そこで何が起きるかを考えます。図2は、その抜き出しの練習そのものです。',
        },
      ],
    },
    {
    ...
    },
    {
      ...
      blocks: [
        {
          kind: 'text',
          text: '仕上げは答案です。科目Bの設問は、大きく「なぜ通信できないか」「この機器は何を見るか」「なぜこの構成か」の3タイプに寄せられます。それぞれ、図のどこを見て何を書くかを型にします。',
        },
    ...
    {
      heading: '見慣れた図で科目Bに挑む',
      blocks: [
        {
          kind: 'text',
          text: '第1章で、PC・ルータ・サーバだけの図から始めました。DNSとDHCPが加わり、VLANで分かれ、境界ができ、冗長になり、無線と電話とクラウドが乗り、最後に監視が付きました。科目Bの構成図は、この20章分の部品でできています。',
        },
        {
          kind: 'text',
          text: 'ここから先は、過去問そのものが教材です。初見の図でも読む順は変わりません。設問が来たら、どの区間の話かを見極めて、層と境界で根拠を組み立てます。この教科書で身につけた読み方が、そのまま科目Bの得点力です。',
        },
      ],
    },
  ],
  takeaways: [
    ...
    '科目Bの構成図は、第1章から見てきた1枚の到達点。図が読めることが、最大の武器です。',
  ],
  checks: [
    {
      question: '科目Bの構成図を読み始めるとき、最初に確かめるものは何か。',
      answer:
