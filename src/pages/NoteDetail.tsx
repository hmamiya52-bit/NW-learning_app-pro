      ...
      },
      {
        ...
        richItems: [
          ...
          [
            ...
            { text: 'パス（', style: 'plain' },
            { text: 'PATH', style: 'red' },
            { text: '）の情報によって経路情報を決定。ASパスには通過したAS番号が書き込まれ、自身のAS番号が含まれていたらループしていると判断し経路情報を破棄', style: 'plain' },
          ],
          [
            { text: 'BGP接続を行うルータ間の経路情報交換：BGPピア（ポート番号：TCP/179）', style: 'plain' },
          ],
          [
            { text: '異なるルーティングプロトコルでは経路交換ができない。OSPFで学習した経路情報をBGPが動作するNWに通知する方法 ⇒ 再配布。再配布された経路を再配布するとループが発生するので、基本的に再々配布はしない設定にする', style: 'plain' },
          ],
          [
            { text: '1つのルータが複数の経路情報を受け取ったときは、ルーティングプロトコルのアドミニストレーティブディスタンス値が小さい方が優先される（優先度高 BGP < OSPF < RIP 優先度低）', style: 'plain' },
          ],
          [
            { text: 'BGPで設定する優先度：MEDとLOCAL_PREF。前者はeBGP（外部）、後者はiBGP（内部）に通知', style: 'plain' },
          ],
          [
            { text: '最適経路選択をする際、ASパスの長さが最も短い経路が優先され、同じ場合、MEDの値が小さい経路が優先される', style: 'plain' },
          ],
          [
            { text: 'BGPにおいて、同じコストの経路を複数設定することで経路の負荷分散を行う技術：BGPマルチパス', style: 'plain' },
          ],
          [
            { text: 'BGPでは、KEEPALIVEメッセージを定期的に送信する。受信しなくなることでピアリングが切断され、AS内の各機器は経路情報をクリア・更新する', style: 'plain' },
          ],
          [
            { text: 'iBGP — 同一AS内のBGP。ネクストホップを自身のIPアドレスに書き換える／別のルータに経路をアドバタイズしない（ループ防止）／通常フルメッシュ構成にする', style: 'plain' },
          ],
          [
            { text: 'ルートリフレクタ：iBGPで経路情報を他のルータに反射 ⇒ iBGPのピアを減らせる。ループ防止のためクラスターIDを設定し、自分のクラスターIDが含まれた経路広告は破棄する', style: 'plain' },
          ],
          [
